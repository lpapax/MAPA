  ...
  return (
    <section className="bg-white rounded-2xl border border-neutral-100 shadow-card overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-50">
        ...
        <Search className="w-4 h-4 text-neutral-300" aria-hidden="true" />
      </div>

      {loading ? (
        <div className="px-5 py-4 text-sm text-neutral-400">Načítám…</div>
      ) : searches.length === 0 ? (
          ...
          {searches.map((s) => (
            <div key={s.id} className="flex items-center gap-3 px-5 py-3">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-neutral-400 truncate">{filtersToText(s.filters)}</p>
              </div>
              <button
