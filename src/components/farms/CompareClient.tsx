...
export function CompareClient({ farms }: CompareClientProps) {
  if (farms.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        <p className="text-neutral-400 text-sm mb-4">Žádné farmy k porovnání. Vyberte je na mapě.</p>
  ...
  const rows: { label: string; render: (farm: Farm) => React.ReactNode }[] = [
    {
      label: 'Kraj',
      render: (f) => <span className="text-sm text-neutral-700">{f.location.kraj}</span>,
    },
    {
      label: 'Město',
      render: (f) => <span className="text-sm text-neutral-700">{f.location.city}</span>,
    },
    {
      label: 'Ověřeno',
      render: (f) => (
        <span className={cn('text-xs font-semibold px-2 py-0.5 rounded-full', f.verified ? 'bg-green-100 text-green-700' : 'bg-neutral-100 text-neutral-500')}>
          {f.verified ? 'Ano' : 'Ne'}
        </span>
      ),
    },
            ...
            {DAY_ORDER.map((day) => {
              ...
              return (
                <div key={day} className="flex gap-2">
                  <span className="text-neutral-400 w-5">{DAY_LABELS[day]}</span>
                  <span className="text-neutral-600">{h ? `${h.open}–${h.close}` : '—'}</span>
                </div>
              )
            })}
          </div>
        )
        : <span className="text-neutral-300 text-sm">Neuvedeno</span>,
    },
  ]

  return (
    <div>
      <Link href="/mapa" className="flex items-center gap-1.5 text-sm text-neutral-500 hover:text-primary-600 transition-colors mb-6">
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse" role="table" aria-label="Porovnání farem">
          <thead>
            <tr>
              <th className="text-left p-4 text-xs font-semibold text-neutral-400 uppercase tracking-wider w-32 bg-neutral-50 rounded-tl-2xl border border-neutral-100">
                Parametr
              </th>
              {farms.map((farm, i) => (
                <th key={farm.id} className={cn('p-4 text-left bg-white border border-neutral-100', i === farms.length - 1 && 'rounded-tr-2xl')}>
                  <Link href={`/farmy/${farm.slug}`} className="font-heading font-bold text-forest text-sm hover:text-primary-600 transition-colors">
                    {farm.name}
                  </Link>
                  <div className="text-xs text-neutral-400 font-normal mt-0.5">{farm.location.city}</div>
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rows.map((row, ri) => (
              <tr key={row.label} className={ri % 2 === 0 ? 'bg-neutral-50/50' : 'bg-white'}>
                <td className="p-4 text-xs font-semibold text-neutral-500 border border-neutral-100 align-top">
                  {row.label}
                </td>
                {farms.map((farm) => (
                  <td key={farm.id} className="p-4 border border-neutral-100 align-top">
                    {row.render(farm)}
                  </td>
                ))}
              </tr>
